...
class Run(object):
    ...
    def train(self, seed,
        ...
        try:
            for y, x_geo, x_sat in data_train:
                self.model.zero_grad()

                _, loss = self.feed_forward(x_geo, x_sat, y,
                                            model=self.model,
                                            cuda=self.cuda,
                                            mode='train')
                ...
                if i % loss_freq == 0 and i != 0:
                    ...
                    # evaluate cv loss
                    y_cv, x_geo_cv, x_sat_cv = data_cv.__next__()
                    outp_t, _ = self.feed_forward(x_geo_cv, x_sat_cv, y_cv,
                                                  model=self.model,
                                                  cuda=self.cuda,
                                                  mode='eval')
                    outp_t.detach_()
    ...
    def predict(self, data_generator):
        ...
        i = 0
        print('\nStart Predicting')
        for y, x_geo, x_sat in data_generator:
            if i % 500 == 0:
                print('\tNo.', i)
            i += 1

            outp, _ = self.feed_forward(x_geo, x_sat, y,
    ...
    @staticmethod
    def feed_forward(x1, x2, y, model, cuda, mode=None):
        if mode == 'train':
            model.train()
            y, x1, x2 = V(y), V(x1), V(x2)
            if cuda:
